...
export default function DashboardPage() {
    ...
    // ✅ FIX: Added logic to identify Yearly budgets (duration > 300 days)
    const isYearlyBudget = (budget: Budget) => {
        const duration = differenceInDays(new Date(budget.endDate), new Date(budget.startDate));
        return duration > 300;
    };
    ...
    // ✅ FIX: Find the current Yearly budget
    const currentYearlyBudget = useMemo(() => {
        return budgets.find(b => {
            const budgetStart = startOfDay(new Date(b.startDate));
            const budgetEnd = endOfDay(new Date(b.endDate));
            return isWithinInterval(currentDate, { start: budgetStart, end: budgetEnd }) && isYearlyBudget(b);
        });
    }, [budgets, currentDate]);
    ...
    // ✅ FIX: State for Yearly selection
    const [selectedYearlyBudget, setSelectedYearlyBudget] = useState<Budget | undefined>(currentYearlyBudget);
    ...
    // ✅ FIX: List of available yearly budgets
    const selectableYearlyBudgets = useMemo(() => {
        return budgets.filter(isYearlyBudget);
    }, [budgets]);
    ...
    const handleUpdateBudget = (newGoal: number, budgetToUpdate: Budget | undefined) => {
        ...
        } else if (isWeeklyBudget(budgetToUpdate)) {
            setSelectedWeeklyBudget({ ...budgetToUpdate, goal: newGoal });
        } else {
            // ✅ FIX: Handle Yearly updates
            setSelectedYearlyBudget({ ...budgetToUpdate, goal: newGoal });
        }
    };
    ...
    const calculateSpent = (budget: Budget, transactions: Transaction[]) => {
        ...
        return transactions
            .filter(t => t.type === 'expense' && isWithinInterval(parseISO(t.date), { start: budgetStart, end: budgetEnd }))
            .reduce((acc, t) => acc + t.amount, 0);
    };

    const budgetForWeeklyCard = useMemo(() => {
        const b = selectedWeeklyBudget || currentWeeklyBudget;
        if (!b) return undefined;
        return { ...b, spent: calculateSpent(b, transactions) };
    }, [selectedWeeklyBudget, currentWeeklyBudget, transactions]);

    const budgetForMonthlyCard = useMemo(() => {
        const b = selectedMonthlyBudget || currentMonthlyBudget;
        if (!b) return undefined;
        return { ...b, spent: calculateSpent(b, transactions) };
    }, [selectedMonthlyBudget, currentMonthlyBudget, transactions]);

    // ✅ FIX: Calculate spent for Yearly Card
    const budgetForYearlyCard = useMemo(() => {
        const b = selectedYearlyBudget || currentYearlyBudget;
        if (!b) return undefined;
        return { ...b, spent: calculateSpent(b, transactions) };
    }, [selectedYearlyBudget, currentYearlyBudget, transactions]);
    ...
    return (
        <>
            ...
            <div className="space-y-6">
                ...
                {/* ✅ FIX: Updated Grid to 3 columns to fit Yearly Budget */}
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    <DashboardBudgetCard
                    ...
                    />
                    {/* ✅ FIX: Added the Yearly Budget Card */}
                    <DashboardBudgetCard
                        title="Selected Yearly Budget"
